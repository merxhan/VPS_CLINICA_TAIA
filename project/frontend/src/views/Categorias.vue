<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="text-2xl font-bold text-primary">{{ $t('nav_categories') }}</h2>
      </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <div v-else class="overflow-x-auto w-full">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-for="categoria in categorias" :key="categoria.id" class="hover:bg-gray-50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ categoria.nombre }}</td>
            <td class="px-6 py-4 text-sm text-gray-500 max-w-lg truncate">{{ categoria.descripcion || '-' }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <button @click="openEditModal(categoria)" class="text-indigo-600 hover:text-indigo-900 mr-4 font-semibold transition-colors">{{ $t('edit') }}</button>
              <button @click="confirmDelete(categoria)" class="text-red-600 hover:text-red-900 font-semibold transition-colors">{{ $t('delete') }}</button>
            </td>
          </tr>
        </tbody>
      </table>
            ...
            <div class="sm:flex sm:items-start">
              <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                <h3 class="text-xl leading-6 font-bold text-gray-900" id="modal-title">
                  {{ form.id ? $t('cat_modal_edit') : $t('cat_modal_title') }}
                </h3>
                <div class="mt-6 space-y-4">
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('services_name') }} *</label>
                    <input v-model="form.nombre" type="text" :class="['input-field', { 'border-red-500 focus:ring-red-500': errors.nombre }]" />
                    <p v-if="errors.nombre" class="mt-1 text-sm text-red-600 font-medium">{{ errors.nombre }}</p>
                  </div>
                  <div>
    ...
    <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-gray-100">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    {{ $t('patients_delete_confirm') }} <span class="font-bold text-gray-700">"{{ categoriaToDelete?.nombre }}"</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-50 px-4 py-4 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-100">
            <button type="button" @click="deleteCategoria" :disabled="isDeleting" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50 transition-colors">
...
const saveCategoria = async () => {
  ...
  if (!form.value.nombre.trim()) {
    ...
    toast.show(t('toast_error_form_incomplete'), 'error');
    return;
  }
  ...
  try {
    if (form.value.id) {
      const res = await api.put<Categoria>(`categorias/${form.value.id}`, payload);
      if (res.ok) {
      ...
      const res = await api.post<Categoria>('categorias', payload);
      if (res.ok) {
...
const deleteCategoria = async () => {
  ...
  try {
    const res = await api.delete(`categorias/${categoriaToDelete.value.id}`);
    if (res.ok) {

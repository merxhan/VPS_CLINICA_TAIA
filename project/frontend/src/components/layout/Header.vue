<template>
  <header class="w-full h-16 bg-white shadow-md flex items-center justify-between px-6 fixed top-0 z-40">
    <div class="flex items-center space-x-4">
      ...
      <button @click="$emit('toggleSidebar')" class="hidden md:block text-gray-600 hover:text-primary focus:outline-none">
    ...
    <div class="flex items-center space-x-4">
      ...
      <button @click="$emit('toggleSidebar')" class="md:hidden text-gray-600 hover:text-primary focus:outline-none">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
      </button>
    </div>
  </header>
